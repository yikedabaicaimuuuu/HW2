...
def compute_LiveOut(CFG, UEVar, VarKill, VarDomain):
    ...
    while Changed:
        Changed = False
        for n in CFG.nodes():
            instruction = get_node_instruction(n)
            
            # Variables defined in the node
            Defined = set()
            if '=' in instruction:
                var, _ = instruction.split('=', 1)
                Defined.add(var.strip())
                
            # Variables used in the node
            Used = set(instruction.split('=')[1].split() if '=' in instruction else instruction.split())
            
            NewLiveOut = (Used - Defined) | (set().union(*[LiveOut[succ] for succ in get_node_successors(CFG, n)]) - Defined)
            
            if n not in LiveOut or NewLiveOut != LiveOut[n]:
                Changed = True
                LiveOut[n] = NewLiveOut
